...
int day_convert(struct date *day);
// int compare_start(struct date *day1,struct date *day2,struct date *day3);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    {
        int pod_day = day_convert(first_pod->when);
        int start = day_convert(start_day);
        int end = day_convert(finish_day);

        if(pod_day <= end && pod_day >= start)
        {
...
int day_convert(struct date *day){
    return day->year * 12 * 31 + day->month * 31 + day->day;
}

// int compare_start(struct date *day1,struct date *day2,struct date *day3)
// {
//     if(day1->year>day2->year&&day1->year<day3)
//     {
//         return 1;
//     }
