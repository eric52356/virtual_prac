    ...
    for(int a=0;a<N_TILES;a++)
	{
		...
		{			
            ...
            {                
                ...
                {
                    //printf("a=%d tag=%d %d %d %d",a,tag[a],)
                    if(tiles1[a]==tiles2[b])
                    {
                        int acount=a-1;
                        while(acount>=0)
                        {
                            if(tiles1[acount]==tiles1[a])
                            {
                                tag[a]=1;
                                //printf("%d",a);
                            }
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    // PUT YOUR CODE HERE
	int i=1;
    int c_farnarkles=0,c_arkles=0;
    int tiles1[N_TILES];	
	while(c_farnarkles!=N_TILES)	
	{	
        c_farnarkles=0;
        c_arkles=0;
        printf("Enter guess for turn %d:",i);
    	assert(read_tiles(tiles1) == N_TILES);
		c_farnarkles=count_farnarkles(hidden_tiles,tiles1);
        c_arkles=count_arkles(hidden_tiles,tiles1);
        printf("%d farnarkles, %d arkles\n",c_farnarkles,c_arkles);
        i++;	
	}
    printf("You win\n");
}
...
void farnarkle_ai(int turn, int previous_guesses[MAX_TURNS][N_TILES],
    ...
    if(turn>=2)
    {
            ...
            guess[0]=8-previous_guesses[turn-1][0];
            guess[1]=8-previous_guesses[turn-1][1];
            guess[2]=8-previous_guesses[turn-1][2];
            guess[3]=8-previous_guesses[turn-1][3];
